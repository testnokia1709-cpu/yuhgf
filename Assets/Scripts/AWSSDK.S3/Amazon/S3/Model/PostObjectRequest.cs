...
namespace Amazon.S3.Model
{
	public class PostObjectRequest : AmazonWebServiceRequest
	{
		...
		internal void WriteFormData(string boundary, Stream outputStream)
		{
			...
			WriteFormDatum(outputStream, S3Constants.PostFormDataAcl, CannedACL, boundary);
			if (_isSetStorageClass)
			{
				WriteFormDatum(outputStream, "x-amz-storage-class", StorageClass, boundary);
			}
			if (_isSetActionStatus)
			{
				WriteFormDatum(outputStream, S3Constants.PostFormDataStatus, ((int)SuccessActionStatus).ToString(CultureInfo.InvariantCulture), boundary);
			}
			...
			{
				string name = (item.Key.StartsWith(S3Constants.PostFormDataXAmzPrefix, StringComparison.Ordinal) ? item.Key : (S3Constants.PostFormDataMetaPrefix + item.Key));
				WriteFormDatum(outputStream, name, item.Value, boundary);
			}
