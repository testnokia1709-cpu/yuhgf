...
namespace Amazon.S3.Util
{
	public static class AmazonS3Util
	{
		...
		{
			byte[] array = CryptoUtilFactory.CryptoInstance.ComputeMD5Hash(Encoding.UTF8.GetBytes(content));
			if (fBase64Encode)
			{
		...
		internal static void SetMetadataHeaders(IRequest request, MetadataCollection metadata)
		{
			...
			{
				request.Headers[key] = metadata[key];
			}
		}
		...
		internal static DateTime? ParseExpiresHeader(string rawValue, string requestId)
		{
			if (!string.IsNullOrEmpty(rawValue))
			{
				...
				{
					throw new AmazonDateTimeUnmarshallingException(requestId, string.Empty, string.Empty, rawValue, string.Format(CultureInfo.InvariantCulture, "The value {0} cannot be converted to a DateTime instance.", rawValue), innerException);
				}
			}
		...
		internal static void ParseAmzRestoreHeader(string header, out bool restoreInProgress, out DateTime? restoreExpiration)
		{
			...
			if (num != -1)
			{
				...
				bool result;
				if (bool.TryParse(header.Substring(num2, num3 - num2), out result))
				{
			...
			if (num != -1)
			{
				...
				DateTime result2;
				if (DateTime.TryParseExact(header.Substring(num4, num5 - num4), "ddd, dd MMM yyyy HH:mm:ss \\G\\M\\T", CultureInfo.InvariantCulture, DateTimeStyles.None, out result2))
				{
		...
		{
			return key.EndsWith("INSTRUCTION_SUFFIX", StringComparison.Ordinal);
		}
	}
}
