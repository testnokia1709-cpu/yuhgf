...
namespace Amazon.Runtime
{
	public abstract class AmazonServiceClient : IDisposable
	{
		...
		public static Uri ComposeUrl(IRequest iRequest)
		{
			...
			string text2 = AWSSDKUtils.UrlEncode(text, true) + stringBuilder;
			Uri obj = ((endpoint.AbsoluteUri.EndsWith("/", StringComparison.Ordinal) || text2.StartsWith("/", StringComparison.Ordinal)) ? new Uri(endpoint.AbsoluteUri + text2) : new Uri(endpoint.AbsoluteUri + "/" + text2));
