	...
	public abstract class RetryPolicy
	{
		...
		private bool IsClockskew(IExecutionContext executionContext, Exception exception)
		{
			...
			if (num || flag)
			{
				...
				DateTime correctedUtcNow = clientConfig.CorrectedUtcNow;
				DateTime serverTime;
				...
				if (flag2)
				{
					...
					if (((timeSpan.Ticks < 0) ? (-timeSpan) : timeSpan) > clockSkewMaxThreshold)
					{
						...
						Logger.InfoFormat("Identified clock skew: local time = {0}, local time with correction = {1}, current clock skew correction = {2}, server time = {3}, service endpoint = {4}.", dateTime, correctedUtcNow, clientConfig.ClockOffset, serverTime, text);
