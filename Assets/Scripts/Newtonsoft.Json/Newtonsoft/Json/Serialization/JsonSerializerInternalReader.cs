...
namespace Newtonsoft.Json.Serialization
{
	internal class JsonSerializerInternalReader : JsonSerializerInternalBase
	{
		...
		private object CreateObject(JsonReader reader, Type objectType, JsonContract contract, JsonProperty member, JsonContainerContract containerContract, JsonProperty containerMember, object existingValue)
		{
			...
			case JsonContractType.Primitive:
			{
				...
				if (Serializer.MetadataPropertyHandling != MetadataPropertyHandling.Ignore && reader.TokenType == JsonToken.PropertyName && string.Equals(reader.Value.ToString(), "$value", StringComparison.Ordinal))
				{
					...
					object result = CreateValueInternal(reader, objectType2, contract2, member, null, null, existingValue);
					reader.ReadAndAssert();
					return result;
				}
				break;
		...
		{
			if ((((member != null) ? member.TypeNameHandling : ((TypeNameHandling?)null)) ?? ((containerContract != null) ? containerContract.ItemTypeNameHandling : ((TypeNameHandling?)null)) ?? ((containerMember != null) ? containerMember.ItemTypeNameHandling : ((TypeNameHandling?)null)) ?? Serializer._typeNameHandling) != TypeNameHandling.None)
			{
				string typeName;
				string assemblyName;
